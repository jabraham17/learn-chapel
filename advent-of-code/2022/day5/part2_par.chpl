...
class Stack {
  ...
  proc moveTo(other: borrowed Stack, n: integral = 1) {
    // var othersLastIdx = other.elms.size;
    // for i in 0..<n {
    //   var x = this.pop();
    //   other.elms.insert(othersLastIdx, x);
    // }
    var x = this.pop(n);
    other.push(x);
...
proc main() {
  ...
  while r.read(m) {
    ...
    begin {
      ...
      fromStack.releaseStack();
      toStack.releaseStack();
    }
  }

  for s in stacks do writeln(s.elms);
